...
class _MonthsWidgetState extends State<MonthsWidget> {
  ...
  void initState() {
    ...
    selectedStartMonthYear = '${monthNames[currentDate.month - 1]} ${currentDate.year}';
    selectedEndMonthYear = selectedStartMonthYear;
    _currentMonths = widget.months;  // Initialize local variable with the initial months value
  }

  @override
  Widget build(BuildContext context) {
    final double progress = (_currentMonths / widget.totalMonths).clamp(0.0, 1.0);
    final DateTime displayStartDate = widget.startDate ?? DateTime(DateTime.now().year, 7, 1);
            ...
            Card(
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: monthYearList.map((monthYear) {
                  return DropdownMenuItem<String>(value: monthYear, child: Text(monthYear));
                }).toList(),
                onChanged: (value) {
                  setState(() {
              ...
              if (progress > 0 && progress < 1)
                Transform.rotate(
                  ...
                  child: Align(
                    ...
                    child: Container(
                      margin: EdgeInsets.only(right: ringRadius - strokeWidth / 2 - 16),
                      width: 32,
                      height: 32,
  ...
  void _updateProgress() {
    if (selectedStartMonthYear != null && selectedEndMonthYear != null) {
      ...
      if (startParts.length == 2 && endParts.length == 2) {
        ...
        final monthsDifference = (endYear * 12 + endMonth) - (startYear * 12 + startMonth);
